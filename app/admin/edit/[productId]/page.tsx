...
export default function Home({
  ...
  const handleInputMulti = useCallback(async () => {
    setUploading(true);
    for (let i = uploadedCount; i < amount; i++) {
      try {
        const imageRef = ref(storage, `product/${files[i - fix].name}`);

        const uploadTask = uploadBytesResumable(imageRef, files[i - fix]);
        ...
        setUploadedCount((prevCount: any) => prevCount + 1);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setData(res);
      setUploadedCount(res.images.length);
      setUploadProgress([...Array(res.images.length).fill(100)]);
      setAmount(res.images.length);
      setFix(res.images.length);
    } catch (error: any) {
  ...
  useEffect(() => {
    if (files.length > 0 && !uploading && amount > 0) {
      handleInputMulti();
    }
            ...
            <div className="w-full mt-2">
              <div className="relative w-full p-2 border-2 border-[#f2f2f2] border-dashed rounded-lg aspect-square">
                {coverBlob ? (
                  ...
                  <Image
                    src={data ? data.cover : "/assets/placeholder.png"}
                    width={500}
                    height={500}
                    alt="placeholder"
                    className="object-cover object-center w-full rounded-lg aspect-square"
                ...
                {uploadedCount < amount && (
                  <div className="relative">
                    <Image
                      ...
                      width={100}
                      height={100}
                      alt={files[uploadedCount - fix].name}
                      className="object-cover w-full rounded-lg aspect-square"
                    />
                    <div
                      className="absolute z-10 top-1/2 left-1/2 radial-progress text-accent"
                      style={{
                        // @ts-ignore
                        "--value": Math.round(
                          uploadProgress[uploadedCount - fix]
                        ),
                        transform: "translate(-45%, -45%)",
                      }}
                      role="progressbar"
                    >
                      {`${Math.round(uploadProgress[uploadedCount - fix])}%`}
                    </div>
                    <div
                      onClick={() => handleDelete(uploadedCount - fix)}
                      className="absolute flex items-center justify-center p-2 bg-red-500 rounded-full cursor-pointer top-1 right-1"
                    >
                      <GoTrash size={16} color="white" />
                    </div>
